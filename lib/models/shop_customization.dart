...
class ShopTemplate {
  ...
  static ShopTemplate _getDefaultTemplate(ScreenShopTheme theme) {
    return ShopTemplate(
      ...
      colors: ShopColors(
        primary: '#${theme.primaryColor.value.toRadixString(16).substring(2)}',
        secondary:
...
class ShopPromotion {
  ...
  // Flash Sale เพิ่มเติม
  final int? flashSaleStock; // จำนวนสินค้าที่ลดราคา (Flash Sale)
  final int flashSaleSold; // จำนวนที่ขายไปแล้ว (Flash Sale)
  final double? originalPrice; // ราคาเดิม (Flash Sale)
  final double? flashSalePrice; // ราคา Flash Sale

  // ระบบติดตาม
  final String? trackingCode; // โค้ดติดตามการใช้งาน
  final String? campaignName; // ชื่อแคมเปญ
  final String? targetAudience; // กลุ่มเป้าหมาย

  // การตั้งค่าขั้นสูง
  final bool requiresApproval; // ต้องการการอนุมัติ
  final bool showInPublicList; // แสดงในรายการสาธารณะ
  final bool allowStacking; // สามารถใช้ร่วมกับโปรโมชั่นอื่น
  final int? maxUsagePerDay; // จำนวนการใช้สูงสุดต่อวัน
  final List<String>? excludedProductIds; // สินค้าที่ไม่ใช้ได้
  final Map<String, dynamic>? customFields; // ฟิลด์เพิ่มเติม
  ...
  factory ShopPromotion.fromMap(Map<String, dynamic> map) {
    return ShopPromotion(
      ...
      flashSaleSold: map['flashSaleSold']?.toInt() ?? 0,
      ...
      excludedProductIds: map['excludedProductIds'] != null
          ? List<String>.from(map['excludedProductIds'])
          : null,
  ...
  String get conditionText {
    switch (type) {
      case PromotionType.percentDiscount:
        String text = 'ลด ${discountPercent!.toInt()}%';
        if (maximumDiscount != null) {
          text += ' (สูงสุด ฿${maximumDiscount!.toInt()})';
        }
        if (minimumPurchase != null) {
          text += ' ซื้อขั้นต่ำ ฿${minimumPurchase!.toInt()}';
        }
        return text;
      case PromotionType.fixedDiscount:
        String text = 'ลด ฿${discountAmount!.toInt()}';
        if (minimumPurchase != null) {
          text += ' ซื้อขั้นต่ำ ฿${minimumPurchase!.toInt()}';
        }
        return text;
      case PromotionType.freeShipping:
        return minimumPurchase != null
            ? 'ซื้อขั้นต่ำ ฿${minimumPurchase!.toInt()}'
            : 'ทุกคำสั่งซื้อ';
  ...
  String? validate() {
    ...
    if (title.length > 100) {
      return 'ชื่อโปรโมชั่นต้องไม่เกิน 100 ตัวอักษร';
    }
    ...
    if (discountCode != null && discountCode!.isNotEmpty) {
      if (discountCode!.length < 3) {
        return 'โค้ดส่วนลดต้องมีอย่างน้อย 3 ตัวอักษร';
      }
      if (discountCode!.length > 20) {
        return 'โค้ดส่วนลดต้องไม่เกิน 20 ตัวอักษร';
      }
      // ตรวจสอบว่าเป็นอักษรภาษาอังกฤษและตัวเลขเท่านั้น
      if (!RegExp(r'^[A-Z0-9]+$').hasMatch(discountCode!)) {
        return 'โค้ดส่วนลดต้องเป็นตัวอักษรภาษาอังกฤษตัวใหญ่และตัวเลขเท่านั้น';
      }
    }
    ...
    switch (type) {
      ...
      case PromotionType.fixedDiscount:
        ...
        if (discountAmount! > 100000) {
          return 'จำนวนเงินส่วนลดต้องไม่เกิน 100,000 บาท';
        }
        break;
      ...
      case PromotionType.buyXGetY:
        ...
        if (buyQuantity! > 100 || getQuantity! > 100) {
          return 'จำนวนสินค้าต้องไม่เกิน 100 ชิ้น';
        }
        break;

      case PromotionType.flashSale:
        if (flashSaleStock != null && flashSaleStock! <= 0) {
          return 'จำนวนสต็อก Flash Sale ต้องมากกว่า 0';
        }
        if (originalPrice != null && originalPrice! <= 0) {
          return 'ราคาเดิมต้องมากกว่า 0';
        }
        if (flashSalePrice != null && flashSalePrice! <= 0) {
          return 'ราคา Flash Sale ต้องมากกว่า 0';
        }
    ...
    if (minimumPurchase != null && minimumPurchase! > 1000000) {
      return 'ยอดซื้อขั้นต่ำต้องไม่เกิน 1,000,000 บาท';
    }
    ...
    if (usageLimit != null && usageLimit! > 1000000) {
      return 'จำนวนการใช้งานต้องไม่เกิน 1,000,000 ครั้ง';
    }
    ...
    if (usageLimitPerUser != null && usageLimitPerUser! > 100) {
      return 'จำนวนการใช้งานต่อคนต้องไม่เกิน 100 ครั้ง';
    }

    if (maxUsagePerDay != null && maxUsagePerDay! <= 0) {
      return 'จำนวนการใช้งานต่อวันต้องมากกว่า 0';
    }

    // ตรวจสอบโค้ดติดตาม
    if (trackingCode != null && trackingCode!.isNotEmpty) {
      if (trackingCode!.length < 5 || trackingCode!.length > 50) {
    ...
    if (terms != null && terms!.length > 1000) {
      return 'เงื่อนไขการใช้งานต้องไม่เกิน 1,000 ตัวอักษร';
    }
  ...
  bool get isReady {
    return validate() == null && isActive;
  }

  // คำนวณจำนวนคงเหลือ Flash Sale
  int? get flashSaleRemaining {
    if (type == PromotionType.flashSale && flashSaleStock != null) {
      return flashSaleStock! - flashSaleSold;
    }
    return null;
  }
